...
``odatse-SXRD`` モジュールは ``sxrdcalc`` を用いて原子位置 :math:`x` や原子の占有率、デバイワラー因子から Rocking curve を計算し、実験で得られた Rocking curve からの誤差を :math:`f(x)` として返す ``Solver`` です。
...
``solver`` セクションおよびサブセクション ``config``, ``post``, ``param``, ``reference`` を利用します。
...
- ``remove_work_dir``
  ...
  説明: true の場合、SXRDソルバー実行後に作業ディレクトリを削除します。デフォルト値は false です。

- ``use_tmpdir``

  形式: bool型

  説明: true の場合、作業ディレクトリを ``/tmp`` (または環境変数 ``TMPDIR`` で指定するディレクトリ) 以下に作成します。ソルバー実行後に作業ディレクトリは削除されます。デフォルト値は false です。
...
ここでは、 ``odatse-SXRD`` で独自に出力するファイルについて説明します。
